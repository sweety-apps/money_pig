...
#import "_SDImageCache.h"
#import "_SDWebImageDownloader.h"

static _SDWebImageManager *instance;

@implementation _SDWebImageManager
...
+ (id)sharedManager
{
    if (instance == nil)
    {
        instance = [[_SDWebImageManager alloc] init];
    }
...
{
    return [[_SDImageCache sharedImageCache] imageFromKey:[url absoluteString]];
}
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options
{
    ...
    [[_SDImageCache sharedImageCache] queryDiskCacheForKey:[url absoluteString] delegate:self userInfo:info];
}
...
- (void)cancelForDelegate:(id<SDWebImageManagerDelegate>)delegate
{
    ...
    {
        _SDWebImageDownloader *downloader = [[downloaders objectAtIndex:idx] retain];
...
- (void)imageCache:(_SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    _SDWebImageDownloader *downloader = [downloaderForURL objectForKey:url];

    if (!downloader)
    {
        downloader = [_SDWebImageDownloader downloaderWithURL:url delegate:self userInfo:info lowPriority:(options & SDWebImageLowPriority)];
...
- (void)imageDownloader:(_SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        _SDWebImageDownloader *aDownloader = [downloaders objectAtIndex:uidx];
    ...
    if (image)
    {
        ...
        [[_SDImageCache sharedImageCache] storeImage:image
