...
CHECKIN_AWARD_DEFINES = [
    CheckInAward(CheckInAwardType.T_NONE,  0,   0.9),
    CheckInAward(CheckInAwardType.T_1MAO,  10,  0.08),
    CheckInAward(CheckInAwardType.T_5MAO,  50,  0.0089),
    CheckInAward(CheckInAwardType.T_3MAO, 30, 0.01),
    CheckInAward(CheckInAwardType.T_1YUAN, 100, 0.001),
    CheckInAward(CheckInAwardType.T_5YUAN, 500, 0.0001)
]
...
class Handler:
    def POST(self):
        ...
        if self.already_checked_in():
            ...
            #roll到了,给账户充值
            if award.money > 0:
                BillingClient.instance().recharge(req.device_id, req.userid, award.money, '抽奖赢得%.1f元' %(award.money/100.0), 0)
            task = TaskCheckIn()
